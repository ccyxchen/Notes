...
ufshcd_check_query_response(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	query_res->response = ufshcd_get_rsp_upiu_result(lrbp->ucd_rsp_ptr) >>
        ...
        dev_err(hba->dev, "%s: line: %d, err = %d\n", __func__, __LINE__, query_res->response);
...
ufshcd_dev_cmd_completion(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	resp = ufshcd_get_req_rsp(lrbp->ucd_rsp_ptr);
        ...
        dev_err(hba->dev, "%s: line: %d, resp = %d\n", __func__, __LINE__, resp);
	switch (resp) {
...
retry:
	...
	wmb();
	if (likely(time_left)) {
	        dev_err(hba->dev, "%s: line: %d, time_left = %d\n", __func__, __LINE__, time_left);
		/*
...
static void ufshcd_wb_probe(struct ufs_hba *hba, u8 *desc_buf)
{
	 ...
	 */
        ...
        ufshcd_map_desc_id_to_length(hba, QUERY_DESC_IDN_DEVICE, &buff_len);
        desc_tmpbuf = kzalloc(buff_len, GFP_KERNEL);
        dev_err(hba->dev, "%s: buff_len = %d\n", __func__, buff_len);
        if (!desc_tmpbuf)
        {
                dev_err(hba->dev, "%s: desc_tmpbuf alloc failed\n", __func__);
        } else {
                ret = ufshcd_read_desc_param(hba, QUERY_DESC_IDN_DEVICE,        \
                                    index, 0, desc_tmpbuf, buff_len);
                if (ret) {
  		        dev_err(hba->dev, "%s: Failed reading descriptor. desc_id %d, desc_index %d, ret %d\n",
  			        __func__, QUERY_DESC_IDN_DEVICE, index, ret);
  	        }

                pm_runtime_get_sync(hba->dev);
                ret = ufshcd_query_attr(hba, UPIU_QUERY_OPCODE_READ_ATTR,    
                        QUERY_ATTR_IDN_CONF_DESC_LOCK, index, 0, &clock); 
                pm_runtime_put_sync(hba->dev); 
                if (ret)
                        dev_err(hba->dev, "%s: Failed reading QUERY_ATTR_IDN_CONF_DESC_LOCK., ret = %d\n", \
                                          __func__, ret);
                if(clock)
                        dev_err(hba->dev, "%s: bConfigDescrLock = %d, can not set the descriptor\n", \
                                        __func__, clock);

                dev_err(hba->dev, "%s: WB_SHARED_ALLOC = %x\n", __func__, 
		                get_unaligned_be32(desc_tmpbuf + DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS));
                dev_err(hba->dev, "%s: bWriteBoosterBufferType = %x\n", __func__, *(desc_tmpbuf + 0x54));
                put_unaligned_be32(0xa000, desc_tmpbuf + DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS);
                ret = ufshcd_query_descriptor_retry(hba, UPIU_QUERY_OPCODE_WRITE_DESC,
  					QUERY_DESC_IDN_DEVICE, index, 0,
  					desc_tmpbuf, &buff_len);
                if (ret) {
                        dev_err(hba->dev, "%s: Failed write descriptor. desc_id %d, desc_index %d, desc_tmpbuf %d, ret %d\n",
  			        __func__, QUERY_DESC_IDN_DEVICE, DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS, 
                                *(int *)(desc_tmpbuf + DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS), ret);
                }
                dev_err(hba->dev, "%s: WB_SHARED_ALLOC = %x\n", __func__, *(int *)(desc_tmpbuf + DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS) );
        }
	...
	if (dev_info->b_wb_buffer_type == WB_BUF_MODE_SHARED) {
		...
		get_unaligned_be32(desc_buf +
                ...
                dev_err(hba->dev, "%s: d_wb_alloc_units = %x\n", __func__, dev_info->d_wb_alloc_units);
